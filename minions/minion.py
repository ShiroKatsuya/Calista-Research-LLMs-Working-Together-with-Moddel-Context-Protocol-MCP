...
from minions.utils import escape_newlines_in_strings, extract_json, clean_json_string, aggressive_json_repair
...
class Minion:
    ...
    def _process_worker_response(
        ...
        if is_privacy:
            ...
            # Format the worker response with appropriate emphasis
            emphasized_response = self._emphasize_worker_response(worker_response[0])
            
            worker_messages.append({"role": "assistant", "content": emphasized_response})
    ...
    def _run_conversation_rounds(
        ...
        for round_idx in range(max_rounds):
            ...
            worker_response, worker_usage, _ = self.local_client.chat(messages=worker_messages)
